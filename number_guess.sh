...
BEST_GAME=$($PSQL "SELECT MIN(number_tries) FROM users INNER JOIN games USING(user_id) WHERE username='$USERNAME'")
...
INSERT_GAME=$($PSQL "INSERT INTO games(number_tries,user_id) VALUES ($GUESS,$USER_ID)")
